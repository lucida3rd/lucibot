...
github：https://github.com/lucida3rd/lucibot3


＜概要＞
python2で作成したmastodonクラウド環境下で動くことを前提にしたbotです。


＜るしぼっと改2との違い＞
・辞書機能をJanome→MeCabに変更。
・タイムラインの取得を連合TLに集約。
・通知一覧の取得に対応。フォロワーでなくても反応できる。
・ハード監視の項目を増やした。
...
---------------------------------
＜前提＞
・mastodon v2.6.1以上（バージョンが異なると一部APIの挙動が異なる場合があるようです）
・CentOS7（Linux系？よく分かりません。少なくともWindows Serverでは動作しません。）
・python2（v2.7で確認）
・cron（もしくはこれに該当するスケジューラ）
・ライブラリ
　Requests-OAuthlib.py（twitter連携機能を利用する場合）
　MeCab（形態素解析エンジン）
・セットアップした鯖のmastodonのアカウント（bot用、master用の2つ）
...
git clone https://github.com/lucida3rd/lucibot3.git lucibot



1.FTPでセットアップしたいフォルダに解凍します。
...
twitterとの機能を使う場合twitterからAPIを取得します。
...
5.viなどで toot/config.txt を書き換えます。
　・UserBot：   botユーザアカウント（ドメイン付き）
　・UserMaster：管理者ユーザアカウント（ドメイン付き）
　・BaseUrl：   インスタンスのドメインURL（例：https://lucida-note.info）
　・twCK、twCS、twAT、twAS
　　Twitter APIで取得した情報。
　　twCK： Consumer key
　　twCS： Consumer secret
　　twAT： Access token
　　twAS： Access token secret
　・MeCab_Path：   4項のライブラリパス
  _reg/registも書き換えます。
　・L17：クライアント名（適当です。連携アプリとして表示されます。）
　・L18：アカウントのドメイン名
　・L19：ID（アカウントのメールアドレス）
　・L20：パスワード（アカウントのパスワード）

6._reg/regist.py を実行します。（mastodonのAPIを取得します）
　ここで _reg フォルダは削除しておきます。

7.スケジューラでbotを定期実行できるように設定します。
　　実行体は２つあります。
　例：cronで1分ごとにbot、1日ごと(0時0分)にハード監視を定期実行する
...
慎重に設定してください。（いちおmastodon v2.4.2には影響なかったです）
...
＜バージョン履歴＞
2019/1/22  ver0.4.2
・ファボ通知の対象を公開・未収載トゥートに限定するようにした。
　未収載はオプションでON/OFFの切り替えができるようにしている。
★configファイルの書式が変わってます。

2019/1/19  ver0.4.1
・連合TLのタグ除去を最適化。
・フォロー、フォロワー情報取得時のバグ修正。
...
2018/12/3  ver0.3.2
・リプライ処理の見直し
　チープバグ対応と、時間差計算の誤りを修正。

2018/12/3  ver0.3.1
・リプライ処理の見直し
　mention以外を新規リプライから除外

2018/12/3  ver0.3.0
・リプライ処理の見直し
　新規リプライが5分以上経ってるものは捨てる
