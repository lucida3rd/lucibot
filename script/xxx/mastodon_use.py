...
###		response = None
		...
		if params_initial['visibility'].lower() not in valid_visibilities:
			wRes['reason'] = "CLS_Mastodon_Use：__status_post：Invalid visibility value"
			return wRes
		...
		if media_ids is not None:
			try:
				...
				wRes['reason'] = "CLS_Mastodon_Use：__status_post：Invalid media：" + str(e)
				return wRes
...
#####################################################
	def __api_request(self, method, endpoint, params={}, files={}, do_ratelimiting=True):
		...
		try:
			...
			wRes['reason'] = "API Responce：Could not complete request：" + str(e)
			return wRes
		
		#############################
		# 応答があったか
		if response_object is None:
			wRes['reason'] = "API Responce：Illegal request："
			return wRes
		
		if response_object.status_code == 404:
			wRes['reason'] = "API Responce：404 ERR："
			return wRes
		elif response_object.status_code == 500:
			wRes['reason'] = "API Responce：500 ERR："
			return wRes
		elif response_object.status_code != 200:
			wRes['reason'] = "API Responce：" + str(response_object.status_code) + " ERR"
			return wRes
